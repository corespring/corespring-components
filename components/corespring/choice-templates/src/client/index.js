    ...
    var ChoiceTemplates = function() {
      ...
      this.choice = function(opts) {
        ...
        return [
          ...
          '          <div>',
                         ...
                         this.inline("radio", "custom", "Custom Feedback", "ng-model='feedback[q.value].feedbackType'"),
          '          </div>',
          '          <div class="clearfix"></div>',
          '          <input class="form-control" type="text" ng-show="feedback[q.value].feedbackType == \'custom\'" ng-model="feedback[q.value].feedback" placeholder="' + placeholderText.selectedFeedback + '"></input>',
          '          <div><span class="feedback-preview" ng-show="feedback[q.value].feedbackType == \'default\'" ng-class="{correct: correctMap[q.value]}">{{correctMap[q.value] ? defaultCorrectFeedback : defaultIncorrectFeedback}}</span></div>',
          ...
          '            <div>',
                          ...
                          this.inline("radio", "custom", "Custom Feedback", "ng-model='feedback[q.value].notChosenFeedbackType'"),
          '            </div>',
          '            <div class="clearfix"></div>',
          '            <input class="form-control" type="text" ng-show="feedback[q.value].notChosenFeedbackType == \'custom\'" ng-model="feedback[q.value].notChosenFeedback" placeholder="' + placeholderText.notSelectedFeedback + '"></input>',
          '            <div><span class="feedback-preview correct" ng-show="feedback[q.value].notChosenFeedbackType == \'default\'">{{defaultNotChosenFeedback}}</span></div>',
          '          </div>',
          '        </div>',
          '      </td>',
          '    </tr>',
      ...
      this.scoring = function() {
        ...
        return [
          ...
          '    <label>If</label> <div class="form-group"><input type="number" min="1" max="{{model.choices.length - 1}}" style="width: 50px" class="form-control {{validClass(scenario)}}" ng-model="scenario.numberOfCorrect"/></div> of correct answers selected, award',
          '    <span class="form-group"><input type="number" min="1" max="99" style="width: 50px" class="form-control" ng-model="scenario.scorePercentage"/>% of full credit</span>',
          '  </div>',
