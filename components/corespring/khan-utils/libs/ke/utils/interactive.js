(function() {
    ...
    var WrappedEllipse = KhanUtil.WrappedEllipse;
    var WrappedLine = KhanUtil.WrappedLine;
    var WrappedPath = KhanUtil.WrappedPath;

    $.fn["interactiveLoad"] = function() {
        kvector = KhanUtil.kvector;
        kpoint = KhanUtil.kpoint;
        kline = KhanUtil.kline;
        WrappedEllipse = KhanUtil.WrappedEllipse;
        WrappedLine = KhanUtil.WrappedLine;
        WrappedPath = KhanUtil.WrappedPath;
    };
    ...
    function Protractor(graph, center, arrowFillColor) {
        ...
        var arrow = graph.raphael.path(
            ...
            "stroke": null,
            "fill": arrowFillColor ? arrowFillColor : KhanUtil.INTERACTIVE
        });
        ...
        $mouseTarget.bind("vmousedown", function(event) {
            isDragging = true;
            arrow.animate({ scale: 1.5, fill: arrowFillColor ? arrowFillColor : KhanUtil.INTERACTING }, 50);

            $(document).bind("vmouseup.rotateHandle", function(event) {
                isDragging = false;

                if (!isHighlight()) {
                    arrow.animate({ scale: 1.0, fill: arrowFillColor ? arrowFillColor : KhanUtil.INTERACTIVE }, 50);
                }
    ...
    function Ruler(graphie, options) {
        ...
        var leftBottomHandle = graphie.addMovablePoint({
            coord: leftBottom,
            normalStyle: {
                fill: options.movablePointColor ? options.movablePointColor : KhanUtil.INTERACTIVE,
                "fill-opacity": 0,
                stroke: options.movablePointColor ? options.movablePointColor : KhanUtil.INTERACTIVE
            },
            highlightStyle: {
                fill: options.movablePointColor ? options.movablePointColor : KhanUtil.INTERACTING,
                "fill-opacity": 0.1,
                stroke: options.movablePointColor ? options.movablePointColor : KhanUtil.INTERACTING
            },
