    ...
    columns.forEach(function(colChoices, colIndex) {
      colChoices.forEach(function(choice, choiceIndex) {
        $(choice).css({
          ...
          left: (this.config.cellWidth + this.config.gutter) * colIndex + 10,
          width: this.config.cellWidth - 30
        });
      }, this);
    }, this);
...
var main = ['$interval',
  function($interval) {
    ...
    function link(scope, elem, attrs) {
      ...
      function cellWidthAsPercentage(gutter){
        var width = cellWidth(gutter);
        var returnValue = Math.min(100, Math.floor(width * 100 / elem.width()));
        return returnValue + "%";
      }
      ...
      function updateView() {
        ...
        if (!isNaN(maxCategoriesPerRow)) {
          ...
          scope.categoryStyle = {
            "width": cellWidthAsPercentage(20)
          };
      ...
      scope.isSolutionPanelOpen = false;

      scope.showSeeSolution = function(response) {
        if (!response) {
          return false;
        }
    ...
    function template() {
      ...
      return [
        ...
        '  <hr/>',
        '  <span ng-if="isEditMode" class="choice-area-edit-help-label">Enter choices below and drag to correct categories above. Choice tiles may be reused unless \"Remove Tile after Placing\" option is selected.</span>',
        choicesTemplate().replace("{flipp}", "!shouldFlip"),
        categoriesTemplate().replace("{flipp}", "shouldFlip").replace("{rowsModel}", "rows"),
      ...
      function seeSolutionTemplate() {
        return [
          '<div class="see-solution" see-answer-panel="" see-answer-panel-expanded="seeSolutionExpanded" ng-show="showSeeSolution(response)">',
...
var category = [function() {
  ...
  function link(scope, elem, attrs) {
    ...
    scope.onCategoryDeleteClicked = function() {
  ...
  return {
    ...
    scope: {
      ...
      notifyDeleteClicked: '&onDeleteCategoryClicked',
  ...
  function template() {
    return [
      ...
      '  >',
      '  <div class="category-label-editor" ng-if="isEditMode">',
      '    <div class="editor" ',
      '       mini-wiggi-wiz="" ',
      ...
      '    ></div>',
      '  </div>',
      '  <div class="category-label-renderer" ng-bind-html-unsafe="label" ng-if="!isEditMode"></div>',
      editControlsDelete(),
    ...
    function deleteTool() {
      return [
        '<li class="delete-icon-button" ng-click="onDeleteCategoryClicked()" tooltip="delete category" tooltip-append-to-body="true" tooltip-placement="bottom">',
...
var choice = ['$sce', 'MiniWiggiScopeExtension', function($sce, MiniWiggiScopeExtension) {
  ...
  function link(scope, elem, attrs) {
    ...
    scope.onDeleteChoiceClicked = function() {
      scope.notifyDeleteClicked({
        choiceId: attrs.choiceId
      });
    };
  ...
  return {
    ...
    scope: {
      ...
      onDragEnd: '&onDragEnd',
      notifyDeleteClicked: '&onDeleteChoiceClicked',
  ...
  function template() {
    return [
      ...
      '      ng-click="onDeleteChoiceClicked()"',
